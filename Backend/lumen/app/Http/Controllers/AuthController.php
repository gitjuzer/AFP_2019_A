...
class AuthController extends Controller {
    ...
    public function login(Request $request) {
        ...
        $result = app('db')->select('SELECT token, username, email, name AS role FROM user INNER JOIN user_role ON user.id = user_role.user_id INNER JOIN role ON role.id = user_role.role_id WHERE username=? AND password=?', [$user, $password]);
